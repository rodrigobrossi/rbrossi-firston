...
export function Billing() {
  ...
  const statusInfo = {
    ...
    trialing:  { label:'Trial', color:'var(--warning)', icon:Clock },
    pending:   { label:'Aguardando pagamento', color:'var(--warning)', icon:Clock },
    none:      { label:'Sem assinatura', color:'var(--text-3)', icon:CreditCard },
    past_due:  { label:'Pagamento atrasado', color:'var(--danger)', icon:CreditCard },
  }
  ...
  return (
    <div>
      <div className="page-header">
        <div><h1 className="page-title">Assinatura</h1><p className="page-sub">FirstOn CRM Pro</p></div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, maxWidth:800 }}>
        ...
        <div className="card" style={{ padding:28 }}>
          ...
          <div style={{ color:'var(--text-2)', fontSize:'.85rem', marginBottom:24 }}>por usuário / mês</div>
          {sub?.next_billing && (
            <div style={{ fontSize:'.8rem', color:'var(--text-2)', marginBottom:20 }}>
              Próxima cobrança: {new Date(sub.next_billing).toLocaleDateString('pt-BR')}
            </div>
          )}
          <button className="btn btn-primary" onClick={handleCharge} disabled={charging} style={{ width:'100%', justifyContent:'center' }}>
            {charging ? 'Gerando PIX…' : <><QrCode size={15}/> Pagar via PIX</>}
          </button>
        </div>
        ...
        <div className="card" style={{ padding:28 }}>
          {qr ? (
            <div style={{ textAlign:'center' }}>
              <p style={{ color:'var(--success)', fontWeight:600, marginBottom:16 }}>✅ PIX gerado!</p>
              <img src={qr.qr_code_url} alt="QR PIX" style={{ width:180, height:180, borderRadius:12, marginBottom:16, border:'4px solid var(--border)' }} />
              <p style={{ fontSize:'.75rem', color:'var(--text-2)', marginBottom:12 }}>Copia e cola:</p>
            ...
            <div style={{ textAlign:'center', color:'var(--text-3)', paddingTop:20 }}>
              ...
              <p style={{ fontSize:'.85rem' }}>Clique em "Pagar via PIX" para gerar o QR code.</p>
              <p style={{ fontSize:'.75rem', marginTop:8 }}>Pagamento confirmado em segundos.</p>
            </div>
          )}
        </div>
        ...
        <div className="card" style={{ gridColumn:'1/-1', padding:24 }}>
          <h3 style={{ fontFamily:'var(--font-display)', fontWeight:700, marginBottom:16 }}>O que está incluso no Pro</h3>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12 }}>
            {[
              'Contatos ilimitados','Calendário inteligente','Pipeline Kanban',
              'WhatsApp integrado','Análise de sentimentos PT-BR','Contratos digitais',
              'Assinatura via PIX','Suporte prioritário','Armazenamento S3',
            ].map(f=>(
